...
void benchmark_thread(std::vector<Entry> &entries, int thread_id, int start_idx, int end_idx)
{
    ...
    sqlite3_exec(db, "PRAGMA read_uncommitted = true", nullptr, nullptr, nullptr);
    // Set the mmap size to 512MB
    sqlite3_exec(db, "PRAGMA mmap_size = 536870912", nullptr, nullptr, nullptr);
    ...
    std::cout << "Thread " << thread_id << ": " << elapsed_ms << " ms (" << (end_idx - start_idx) / (elapsed_ms / 1000.0) / 1000 << " kops/sec)" << std::endl;
