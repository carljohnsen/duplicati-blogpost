...
namespace sqlite_bench
{
    ...
    public class SQLiteSelectParallelBenchmark : SQLiteBenchmarkParallel
    {
        ...
        private SqliteCommand m_createIndexCommand;
        private SqliteCommand m_insertBlocksetManagedCommand;
        private readonly List<SqliteCommand> m_selectCommands = [];
        ...
        public async Task GlobalSetup()
        {
            ...
            using var con = CreateConnection(Backend);
            ...
            m_insertBlocksetManagedCommand = CreateCommand(con, SQLQeuriesOriginal.InsertBlocksetManaged);
            ...
            m_createIndexCommand = CreateCommand(con, SQLQeuriesOriginal.CreateIndex);
            ...
            CreateConnections(Backend, Parallelism);
        ...
        public async Task SelectBenchmark()
        {
            ...
            //Parallel.For(0, Parallelism, async i =>
            var tasks = Enumerable.Range(0, Parallelism).Select(i => Task.Run(async () =>
            {
                ...
                var con = cons[i];
                //transactions[i] ??= con.BeginTransaction();
                using var cmd = con.CreateCommand();
